...
class AccountPeriod(models.Model):
    ...
    special = fields.Boolean(
        "Opening/Closing Period", help="These periods can overlap."
    )
    date_start = fields.Date(
        "Start of Period", required=True, states={"done": [("readonly", True)]}
    )
    date_stop = fields.Date(
        "End of Period", required=True, states={"done": [("readonly", True)]}
    )
