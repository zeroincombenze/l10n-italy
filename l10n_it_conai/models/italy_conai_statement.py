...
class ItalyConaiStatement(models.Model):
    ...
    def compute_statement(self):
        ...
        def manage_invoice_line(statement, invoice, inv_line,
                                conai_category_id, weight,
                                statement_line_ids):
            ...
            vals = {
                ...
                'conai_exemption_id':
                    invoice.conai_exemption_id and
                    invoice.conai_exemption_id.id,
        ...
        for statement in self:
            ...
            domain = [('date_invoice', '>=', statement.date_from),
            ...
            for invoice in self.env['account.invoice'].search(
                ...
                for inv_line in invoice.invoice_line_ids:
                    if (not inv_line.conai_category_id and
                            not inv_line.conai_category2_id):
                        continue
                    if inv_line.conai_category_id:
                        ...
                        if inv_line.product_id:
                            weight2 = (
                                (inv_line.product_id.weight2 or
                                 inv_line.product_id.product_tmpl_id.weight2) *
                                inv_line.quantity)
                            category2 = (
                                inv_line.product_id.conai_category2_id or
                                inv_line.product_id.product_tmpl_id.conai_category2_id)
                        statement_line_ids = manage_invoice_line(
                            statement, invoice, inv_line,
    ...
    def compute_category_total(self):
        ...
        for statement in self:
            category_total = {}
            category_line_ids = sorted(
                [x.id for x in statement.conai_category_ids])
            for line in statement.conai_line_ids:
                hash = '%s-%s' % (line.conai_category_id.code,
            ...
            for category in category_total.keys():
                vals = {
                    'statement_id': statement.id,
                    'conai_category_id': category_total[category]['conai_category_id'].id,
                    'conai_exemption_id': category_total[category]['conai_exemption_id'].id,
                ...
                if len(category_line_ids):
                    category_line = category_model.browse(
                        category_line_ids.pop())
...
class ItalyConaiStatementLine(models.Model):
    ...
    statement_id = fields.Many2one(
        'italy.conai.statement', string='CONAI statement')
    ...
    weight = fields.Float(string="CONAI Weight",
