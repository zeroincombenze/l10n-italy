            ...
            if invoice.customs_doc_type == "forwarder_invoice":
                ...
                if not advance_customs_vat_line:
                    raise orm.except_orm(
                        _("Error"),
                        _(
                            "Forwarder invoice %s does not have lines with 'Adavance Customs Vat'"
                        )
                        % invoice.number,
                    )
                    ...
                    for boe_line in bill_of_entry.invoice_line:
                        ...
                        if boe_line.invoice_line_tax_id:
                            ...
                            tax_code_id = (
                                ...
                                and boe_line.invoice_line_tax_id[0].base_code_id.id
                                or False
                            )
                        line_vals = {
                ...
                move_line_obj.reconcile_partial(
                    cr, uid, reconcile_ids, type="auto", context=context
                )
        return res
        ...
        for invoice in self.browse(cr, uid, ids, context):
            if invoice.bill_of_entry_storno_id:
                account_move_obj.button_cancel(
                    cr, uid, [invoice.bill_of_entry_storno_id.id], context=context
                )
