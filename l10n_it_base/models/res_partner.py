...
class ResPartner(models.Model):
    ...
    def name_get(self):
        ...
        if (
            not self._context.get("html_format")
            and not self._context.get("show_address_only")
            and
    ...
    def _build_where_city(self, level=None, ign_city=None):
        ...
        if self.zip:
            zip = "%s%s" % (self.zip[0 : len(self.zip) - level], "%" * level)
    ...
    def onchange_vat(self):
        res = {}
        if self.vat:
            ids = self.search([("vat", "=", self.vat), ("parent_id", "!=", False)])
            if ids:
                name = self.browse(ids[0].id).name
                res["warning"] = {
                    "title": _("Warning"),
                    "message": _("Found another partner with same vat: " "%s" % name),
                }
        return res
