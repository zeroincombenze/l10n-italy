        ...
        for invoice in self:
            if invoice.split_payment:
                ...
                tax_line = invoice.move_id.line_ids.filtered(
                    ...
                    and self.company_id.id == x.company_id.id
                    and x.line_type == "tax"
                )
                ...
                for tl in tax_line:
                    ...
                    tranfer = line_model.with_context(check_move_validity=False).create(
                        transfer_line_vals
                    )

                    if not tranfer:
                        raise UserError(
                            "Movimento Iva in scissione pagamenti " "non impostato."
                        )
                    # end if
