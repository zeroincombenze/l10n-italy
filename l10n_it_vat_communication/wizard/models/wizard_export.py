...
class WizardVatCommunication(models.TransientModel):
    ...
    def old_get_dati_fattura_header(
        self, cr, uid, commitment_model, commitment, context=None
    ):
        context = context or {}
        fields = commitment_model.get_xml_fattura_header(cr, uid, commitment, context)
    ...
    def get_dati_fattura_header(self, commitment_model, commitment):
        return self.old_get_dati_fattura_header(
            cr=self.env.cr,
    ...
    def old_get_cedente_prestatore(self, cr, uid, fields, dte_dtr_id, context=None):
        ...
        CedentePrestatore.AltriDatiIdentificativi = self.old_get_name(
            cr, uid, fields, dte_dtr_id, partner_type, context
        )
        return CedentePrestatore

    @api.model
    def get_cedente_prestatore(self, fields, dte_dtr_id):
        return self.old_get_cedente_prestatore(
            cr=self.env.cr,
            uid=self.env.user.id,
            fields=fields,
            dte_dtr_id=dte_dtr_id,
    ...
    def old_get_cessionario_committente(
        self, cr, uid, fields, dte_dtr_id, context=None
    ):
        if dte_dtr_id == "DTE":
        ...
        partner.AltriDatiIdentificativi = self.old_get_name(
            cr, uid, fields, dte_dtr_id, partner_type, context
        )
        return partner
    ...
    def get_cessionario_committente(self, fields, dte_dtr_id):
        return self.old_get_cessionario_committente(
            cr=self.env.cr,
            uid=self.env.user.id,
            fields=fields,
            dte_dtr_id=dte_dtr_id,
    ...
    def old_get_dte_dtr(
        self, cr, uid, commitment_model, commitment, dte_dtr_id, context=None
    ):
        context = context or {}
    ...
    def export_vat_communication_DTE(self):
        return self.old_export_vat_communication_DTE(
            cr=self.env.cr, uid=self.env.user.id, ids=self.ids, context=self.env.context
        )
    ...
    def export_vat_communication_DTR(self):
        return self.old_export_vat_communication_DTR(
            cr=self.env.cr, uid=self.env.user.id, ids=self.ids, context=self.env.context
        )
    ...
    def export_vat_communication(self):
        return self.old_export_vat_communication(
            cr=self.env.cr, uid=self.env.user.id, ids=self.ids, context=self.env.context
        )
