...
class AddPickingToDdt(models.TransientModel):
    ...
    def check_4_delivery_value(self, picking, fieldname, condition_help):
        ...
        # otherwise check in sale order of picking (if exists)
            ...
            and picking.sale_id[so_fieldname] != self.ddt_id[pp_fieldname]
        ):
            raise UserError(
                _(
    ...
    def add_to_ddt(self):
        ...
        for picking in pickings:
            ...
            if picking.ddt_ids and self.ddt_id.id in [d.id for d in picking.ddt_ids]:
                raise UserError(_("Picking %s already in ddt") % picking.name)
            elif current_ddt_shipping_partner != self.ddt_id.partner_shipping_id:
                raise UserError(
