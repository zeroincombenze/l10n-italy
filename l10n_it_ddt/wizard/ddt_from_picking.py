...
class DdTFromPickings(models.TransientModel):
    ...
    def create_ddt(self):
        def get_delivery_value(vals, picking, fieldname, condition_help):
            ...
            if not vals.get(fieldname):
                ...
                # field from delivery.carrier?
                    ...
                    dc_fieldname
                    and picking.sale_id
                    and picking.sale_id.carrier_id
                    and picking.sale_id.carrier_id[dc_fieldname]
                ):
                    if fieldname.endswith("_id"):
                        vals[pp_fieldname] = picking.sale_id.carrier_id[dc_fieldname].id
                    else:
                        vals[pp_fieldname] = picking.sale_id.carrier_id[dc_fieldname]
                # field from stock.ddt.type ?
                elif (
                    dt_fieldname and picking.ddt_type and picking.ddt_type[dt_fieldname]
                ):
                    if fieldname.endswith("_id"):
                ...
                # field from partner ?
                    ...
                    and picking.partner_id[rp_fieldname]
                ):
                    if fieldname.endswith("_id"):
        ...
        for picking in self.picking_ids:
            ...
            # self.env["stock.picking.package.preparation"].check_linked_picking(picking)
        ...
        for picking in self.picking_ids:
            vals = get_delivery_value(vals, picking, "ddt_type_id", _("ddt type"))
        if not vals.get("ddt_type_id"):
            ddt_type = self.env["stock.ddt.type"].search([], limit=1)
            if ddt_type:
                vals["ddt_type_id"] = ddt_type[0].id
        for picking in self.picking_ids:
            vals = get_delivery_value(vals, picking, "carrier_id", _("delivery method"))
        for picking in self.picking_ids:
            vals = get_delivery_value(vals, picking, "ddt_carrier_id", _("carrier"))
            vals = get_delivery_value(vals, picking, "show_price", _("show price"))
            vals = get_delivery_value(vals, picking, "note", _("note"))
            vals = get_delivery_value(
                vals, picking, "carriage_condition_id", _("carriage condition")
            )
            vals = get_delivery_value(
                vals, picking, "goods_description_id", _("goods description")
            )
            vals = get_delivery_value(
                vals, picking, "transportation_reason_id", _("transportation reason")
            )
            vals = get_delivery_value(
                vals, picking, "transportation_method_id", _("transportation method")
            )
