    ...
    def _get_statement_line_list(self, liq):
        ...
        for debit_line in liq.debit_vat_account_line_ids:
            if (debit_line.tax_id.vsc_exclude_operation or
                    (debit_line.tax_id.nature_id and
                     debit_line.tax_id.nature_id.code == 'N1')):
                continue
            debit_lines |= debit_line
        for credit_line in liq.credit_vat_account_line_ids:
            if (credit_line.tax_id.vsc_exclude_operation or
                    (credit_line.tax_id.nature_id and
                     credit_line.tax_id.nature_id.code == 'N1')):
                continue
    ...
    def compute_from_liquidazioni(self):
        ...
        for quadro in self:
            ...
            for liq in quadro.liquidazioni_ids:
                ...
                liquidazioni_credit_line_ids += [x.id
                                                 for x in credit_lines]

            # credito/debito periodo precedente
            if liq_1:
                quadro.debito_periodo_precedente = liq_1.\
                    previous_debit_vat_amount
                if liq.previous_year_credit:
                    quadro.credito_anno_precedente = liq_1.\
                        previous_credit_vat_amount
                else:
                    quadro.credito_periodo_precedente = liq_1.\
                        previous_credit_vat_amount
