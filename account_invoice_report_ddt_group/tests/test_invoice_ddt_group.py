...
class TestDdt(TransactionCase):
    ...
    def setUp(self):
        ...
        self.carriage_condition_PA = self.env.ref("l10n_it_ddt.carriage_condition_PA")
        self.goods_description_CAR = self.env.ref("l10n_it_ddt.goods_description_CAR")
        self.goods_description_BAN = self.env.ref("l10n_it_ddt.goods_description_BAN")
    ...
    def test_ddt_group(self):
        ...
        wizard = (
            self.env["ddt.from.pickings"]
            .with_context({"active_ids": [p.id for p in order1.picking_ids]})
            .create({})
        )
        ...
        wizard = (
            self.env["ddt.from.pickings"]
            .with_context({"active_ids": [p.id for p in order2.picking_ids]})
            .create({})
        )
        res = wizard.create_ddt()
        ...
        # test invoice
        ddt1.transportation_reason_id = self.transportation_reason_VEN.id
        ddt2.transportation_reason_id = self.transportation_reason_VEN.id
        invoice_wizard = (
            self.env["ddt.create.invoice"]
            .with_context({"active_ids": [ddt1.id, ddt2.id]})
            .create({})
        )
        ...
        ddt1_key = "%s - %s" % (
            ddt1.ddt_number,
            "%s/%s/%s" % (ddt1_date.day, ddt1_date.month, ddt1_date.year),
        )
        ddt2_date = fields.Date.from_string(ddt2.date)
        ddt2_key = "%s - %s" % (
            ddt2.ddt_number,
            "%s/%s/%s" % (ddt2_date.day, ddt2_date.month, ddt2_date.year),
        )
        self.assertEqual(
