                            ...
                            if tax_line.tax_code_id.withholding_type:
                                if voucher.type != 'payment':
                                    raise orm.except_orm(_('Error'), _(
                                        'Can\'t handle withholding tax with voucher of type other than payment'))
                                if not tax_line.tax_code_id.withholding_payment_term_id:
                                    raise orm.except_orm(_('Error'), _(
                                        'The tax does not have an associated Withholding Payment Term'))
                                if not invoice.company_id.withholding_journal_id:
                                    raise orm.except_orm(_('Error'), _(
                                        'The company does not have an associated Withholding journal'))
                                due_list = term_pool.compute(
                                    cr, uid,
                                    tax_line.tax_code_id.withholding_payment_term_id.id, line.amount,
                                    date_ref=voucher.date or invoice.date_invoice, context=context)
                                if len(due_list) > 1:
                                    raise orm.except_orm(
                                        _('Error'),
                                        _(
                                            'The payment term %s has too many due dates')
                                ...
                                new_move = {
                                    ...
                                    'line_id': [
                                        (0, 0, {
                                            'name': _(
                                                'Giro contabile ritenuta acconto - ') + invoice.number,
                                            'account_id':
                                                tax_line.account_collected_id.id,
                                ...
                                voucher.write({'withholding_move_ids': [(
                                    4, move_id)]})
        return res
