# Copyright 2018-15 Alessandro Camilli (<http://www.openforce.it>)
# Copyright 2018-26 Lorenzo Battistini (https://github.com/eLBati)
# Copyright 2019-26 Giovanni - GSLabIt
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl.html).
{
    "name": "Italian Withholding Tax",
    "version": "12.0.2.1.0_13",
    "category": "Account",
    "summary": "Italian Withholding Tax",
    "author": (
        "Openforce,Odoo Italia Network,Odoo Community Association (OCA)"
        ",SHS-AV s.r.l.,Openforce, Odoo Italia Network,"
    ),
    "website": "https://www.zeroincombenze.it/fatturazione-elettronica",
    "development_status": "Alpha",
    "license": "AGPL-3",
    "depends": [
        "account",
        "l10n_it_fatturapa",
        "l10n_it_account_zma",
    ],
    "data": [
        "views/account.xml",
        "views/report_invoice.xml",
        "views/withholding_tax.xml",
        "security/ir.model.access.csv",
        "security/security.xml",
    ],
    "qweb": ["static/src/xml/account_payment.xml"],
    "maintainer": "Antonio M. Vigliotti <antoniomaria.vigliotti@gmail.com>",
    "installable": True,
    "pre_init_hook": "check_4_depending",
}
