...
class account_vat_period_end_statement(orm.Model):
    ...
    def _compute_interest_vat_amount(
            self, cr, uid, ids, field_name, arg, context):
        res = {}
        interest_vat_amount = 0.0
        for i in ids:
            statement = self.browse(cr, uid, i)
            if statement.authority_vat_amount > 0.0 and statement.period_ids[
                    0].fiscalyear_id.quarter_vat_period:
    ...
    # return the ids of the move lines which has the same account
    # than the statement
    # whose id is in ids
    ...
    def _get_statement_from_move(self, cr, uid, ids, context=None):
        move = {}
        statement_ids = []
        for move in self.pool['account.move'].browse(
                cr, uid, ids, context=context):
            found_ids = self.pool['account.vat.period.end.statement'].search(
    ...
    def _get_statement_from_reconcile(self, cr, uid, ids, context=None):
        move = {}
        for r in self.pool['account.move.reconcile'].browse(
                cr, uid, ids, context=context):
    ...
    def _get_credit_line(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool['statement.credit.account.line'].browse(
                cr, uid, ids, context=context):
    ...
    def _get_debit_line(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool['statement.debit.account.line'].browse(
                cr, uid, ids, context=context):
            result[line.statement_id.id] = True
        return result.keys()
    ...
    def _get_generic_line(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool['statement.generic.account.line'].browse(
                cr, uid, ids, context=context):
            result[line.statement_id.id] = True
        return result.keys()
        ...
        for statement in self.browse(cr, uid, ids, context=context):
            ...
            if statement.move_id:
                for m in statement.move_id.line_id:
                    temp_lines = []
                    if m.reconcile_id:
                        temp_lines = map(lambda x:
                                         x.id, m.reconcile_id.line_id)
    ...
    _columns = {
        ...
        'reconciled': fields.function(
            ...
            store={
                'account.vat.period.end.statement': (
                    ...
                    ids, None, 50),
                'account.move.line': (
                    _get_statement_from_line,
                    None, 50),
                'account.move.reconcile': (
                    _get_statement_from_reconcile,
                    None, 50), },
        ...
        'residual': fields.function(
            _amount_residual, digits_compute=dp.get_precision(
            ...
            store={'account.vat.period.end.statement':
                   ...
                   'statement.generic.account.line': (
                       _get_generic_line, [
                           'amount', 'statement_id'], 50),
                   'account.move': (
                       _get_statement_from_move, None,
                       50),
                   'account.move.line': (
                       _get_statement_from_line, None,
                       50),
                   'account.move.reconcile': (
                       _get_statement_from_reconcile,
                       None, 50), },
        ...
        'interest_rate': fields.float(
            'Interest rate for quarter', digits_compute=dp.get_precision(
                'Account')),
        'interest_amount': fields.function(
            _compute_interest_vat_amount, method=True,
            string='Authority VAT Interest Amount'),
        'quarter_vat_period': fields.boolean('Quarter VAT period'),
        ...
        'statement_ids': fields.one2many(
            'account.vat.period.end.statement', 'endyear_statement_id',
            'End of year Statement childs'),
        'endyear_period_ids': fields.function(
            _get_endyear_periods, relation='account.period', type="many2many",
            string='End of year periods'),
    }
    ...
    def create_move(self, cr, uid, ids, context=None):
        ...
        for statement in self.browse(cr, uid, ids, context):
            ...
            move_data = {'name': _('VAT statement') + ' - ' + statement.date,
                         'date': statement.date,
            ...
            for debit_line in statement.debit_vat_account_line_ids:
                debit_vat_data = {'name': _('Debit VAT'),
            ...
            if statement.previous_credit_vat_amount:
                if not statement.previous_credit_vat_account_id:
                    raise orm.except_orm(_('Error'),
                                         _(
                        'You have a Previous Credits VAT of %s and need to set Previous Credits VAT') % statement.previous_credit_vat_amount)

                previous_credit_vat_data = {'name': _('Previous Credits VAT'),
                                            'account_id':
                                                statement.previous_credit_vat_account_id.id,
                                            'move_id': move_id,
                                            'journal_id':
            ...
            if statement.previous_debit_vat_amount:
                if not statement.previous_debit_vat_amount:
                    raise orm.except_orm(_('Error'),
                                         _(
                        'You have a Previous Debit VAT of %s and need to set Previous Debit VAT') % statement.previous_debit_vat_amount)

                previous_debit_vat_data = {'name': _('Previous Debits VAT'),
                                           'account_id':
                                               statement.previous_debit_vat_account_id.id,
                                           'move_id': move_id,
                                           'journal_id':
                                               statement.journal_id.id,
                                           'debit': 0.0,
            ...
            for generic_line in statement.generic_vat_account_line_ids:
                generic_vat_data = {'name': _('Other VAT Credits / Debits'),
            ...
            if statement.interest_amount > 0.0:
                ...
                if not company.interest_quarter_vat_account_id:
                    raise orm.except_orm(_('Error'),
                                         _(
                        'In the company is not configured the account for interest amount'))
                interest_amount_data = {'name': _(
                    'Interest Amount for Quarter VAT'),
                    'account_id':
                    company.interest_quarter_vat_account_id.id,
                    'move_id': move_id,
                    'journal_id': statement.journal_id.id,
                    'debit': math.fabs(
            ...
            end_debit_vat_data = {'name': _('Tax Authority VAT'),
                                  ...
                                  'partner_id':
                                      statement.authority_partner_id.id,
                                  'move_id': move_id,
                                  'journal_id': statement.journal_id.id,
                                  'date': statement.date,
                                  'period_id': period_ids[0], }
            if statement.authority_vat_amount > 0:
                ...
                end_debit_vat_data['credit'] = math.fabs(
                    statement.authority_vat_amount) + math.fabs(
                        statement.interest_amount)
                if statement.payment_term_id:
                    ...
                    if len(due_list) == 0:
                        raise orm.except_orm(_('Error'),
                                             _(
                                                 'The payment term %s does not have due dates') % statement.payment_term_id.name)
    ...
    def open_chart_of_taxes(self, cr, uid, ids, context=None):
        ...
        for statement in self.browse(cr, uid, ids, context):
            ...
            if len(period_ids)> 1:
                raise orm.except_orm(_('Error'), _(
                    'Too many periods for date %s') % str(statement.date))
    ...
    def add_periods(self, cr, uid, ids, context=None):
        for statement in self.browse(cr, uid, ids, context):
            ...
            fiscalyears = self.pool['account.fiscalyear'].search(cr, uid, [(
                'date_start', '<=', st_date), (
                'date_stop', '>=', st_date)], limit=1)
            fiscalyear = self.pool['account.fiscalyear'].browse(
                cr, uid, fiscalyears[0])
            period_obj = self.pool['account.period']
            periods = period_obj.search(cr, uid, [('special', '=', False,), (
                'fiscalyear_id', '=', fiscalyear.id)], context=context)
            statements = self.search(cr, uid, [(
                'period_ids', 'in', periods)], context=context)
    ...
    def compute_amounts(self, cr, uid, ids, context=None):
        ...
        for statement in self.browse(cr, uid, ids, context):
            ...
            if prev_statement_ids:
                prev_statement = self.browse(cr, uid, prev_statement_ids[len(
                    prev_statement_ids) - 1], context)
                if prev_statement.residual > 0 and prev_statement.authority_vat_amount > 0:
                    statement.write({'previous_debit_vat_amount':
                                     prev_statement.residual})
                elif prev_statement.authority_vat_amount < 0:
                    statement.write({'previous_credit_vat_amount': -
            ...
            for debit_tax_code_id in debit_tax_code_ids:
                ...
                debit_line_ids.append(
                    {'account_id': debit_tax_code.vat_statement_account_id.id,
                     ...
                     'amount':
                         not debit_tax_code.is_base and total * debit_tax_code.vat_statement_sign or 0.0,
                     'amount_base':
                         debit_tax_code.is_base and total * debit_tax_code.vat_statement_sign or 0.0, })
            ...
            for credit_tax_code_id in credit_tax_code_ids:
                ...
                credit_line_ids.append({'account_id':
                                        credit_tax_code.vat_statement_account_id.id,
                                        'tax_code_id': credit_tax_code.id,
                                        'amount':
                                            not credit_tax_code.is_base and total * credit_tax_code.vat_statement_sign or 0.0,
                                        'amount_base':
                                            credit_tax_code.is_base and total * credit_tax_code.vat_statement_sign or 0.0, })
...
class statement_debit_account_line(orm.Model):
    ...
    _columns = {'account_id': fields.many2one(
        ...
        'amount': fields.float(
        'Amount', digits_compute=dp.get_precision(
...
class account_tax_code(orm.Model):
    ...
    _columns = {'vat_statement_account_id': fields.many2one(
        'account.account', "Account used for VAT statement"),
        'vat_statement_type': fields.selection([('credit', 'Credit'), (
            'debit', 'Debit')], 'Type',
    ...
    _defaults = {'vat_statement_type': 'debit',
...
class res_company(orm.Model):
    ...
    _columns = {
        'interest_quarter_vat_account_id': fields.many2one(
            'account.account', 'Interest account for quarter VAT'),
    }
