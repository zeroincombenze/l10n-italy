...
class ResPartner(models.Model):
    ...
    def _split_last_name(self):
        for partner in self:
            lastname, firstname = self._split_last_first_name(partner=partner)
            if lastname:
    ...
    def _split_last_first_name(self, partner=None, name=None, splitmode=None):
        if partner:
            ...
            splitmode = self._default_splitmode()
        if not isinstance(name, string_types) or not isinstance(
            splitmode, string_types
        ):
            return "", ""
    ...
    splitmode = fields.Selection(SPLIT_MODE, "First Last format", default="LF")
    ...
    split_next = fields.Boolean(
        "◒ ⇔ ◓", default=False, help="Check for change first/last name format"
    )
