...
class account_tax(osv.osv):
    ...
    def get_account_tax_by_tax_code(self, tax_code):
        ...
        raise osv.except_osv(
            _("Error"), _("No taxes associated to tax code %s") % str(tax_code.name)
        )
    ...
    def compute_all(
        ...
        res = super(account_tax, self).compute_all(
            cr, uid, taxes, price_unit, quantity, product, partner, force_excluded
        )
        ...
        if len(tax_list) == 2:
            for tax in tax_list:
                if tax.get(
                    ...
                    tax_total = float(
                        Decimal(str(tax["balance"])).quantize(
                            Decimal("1." + precision * "0"), rounding=ROUND_HALF_UP
                        )
                    )
    ...
    def tax_difference(self, cr, uid, cur, tax_grouped):
        ...
        for inv_tax in tax_grouped.values():
            if inv_tax["tax_code_id"]:
                ...
                raise osv.except_osv(
                    _("Error"), _("No tax codes for invoice tax %s") % inv_tax["name"]
                )
                    ...
                    if inv_tax_2["base_code_id"] and not inv_tax_2["tax_code_id"]:
                        ...
                        if main_tax.id == tax.id:
                            ...
                            # se risulta una mancanza, la aggiungo alla parte indetraibile
                                ...
                                inv_tax_2["amount"] = (
                                    inv_tax_2["amount"] + tax_difference
                                )
