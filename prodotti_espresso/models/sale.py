        ...
        if ids:
            ...
            for order in orders:
                if order.order_line.filtered(
                    lambda x: (
                        x.product_espresso is True and x.product_id.type != "service"
                    )
                ):
                    to_process.append(order)
            if not to_process:
                raise UserError(
                    "No product with express flag was found in sale orders!"
                )
                ...
                if order.picking_ids:
                    for pick in order.picking_ids:
                        if pick.state == "assigned":
                            ...
                            #     pick.delivery_note_id
                            #     and pick.delivery_note_id.state == "draft"
                            # ):
                            if not pick.ddt_ids:
                                ...
                                if product_line:
                                    ...
                                    for pl in product_line:
                                        if pl.quantity_done == 0:
                                            pl.write(
                                                {"quantity_done": pl.product_uom_qty}
                                            )
        if picks_lines:
            for pick_id in picks_lines:
                ...
                if picking._get_overprocessed_stock_moves():
                    raise UserError(
                        _(
                            "Alcuni prodotti hanno una quantità superiore a "
                ...
                if picking.state != "done":
                    raise UserError(
                        _(
                            "La consegna {cn} risulta in stato anomalo {st} "
            ...
            raise UserError("Non sono state rilevate consegne in stato " "pronto.")
