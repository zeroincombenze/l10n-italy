...
class AccountTax(common.TransactionCase):
    ...
    def test_account_tax(self):
        ...
        self.assertEqual(tax._get_tax_amount(), 22.0, msg="Invalid rate for 22v")
        tax = self.model_browse(model, "external.22a")
        self.assertEqual(tax._get_tax_amount(), 22.0, msg="Invalid rate for 22a")
        tax = self.model_browse(model, "external.2280a")
        self.assertEqual(tax._get_tax_amount(), 22.0, msg="Invalid rate for 2280a")
        ...
        for (xref, name, test_base, test_vat) in (
            ...
            (tax_name, base, vat, deductible, undeductible) = tax._compute_totals_tax(
                ctx
            )
            self.assertEqual(tax_name, name, msg="Invalid name for %s" % xref)
            # self.assertEqual(
            #     base,
            #     test_base,
            #     msg='Invalid name for %s' % xref)
