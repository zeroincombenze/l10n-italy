...
class print_prima_nota_cassa(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        data["form"].get("used_context", {}).copy()
    ...
    def get_children_accounts(self, account):
        ...
        cash_bank_accounts = [
            journal_obj.browse(self.cr, self.uid, j).default_credit_account_id.id
            for j in cash_bank_journals
        ] + [
            journal_obj.browse(self.cr, self.uid, j).default_debit_account_id.id
            for j in cash_bank_journals
        ]

        ids_acc = [
            acc
            for acc in self.pool.get("account.account")._get_children_and_consol(
                self.cr, self.uid, account.id
            )
            if acc in cash_bank_accounts
        ]

        account.currency_id and account.currency_id or account.company_id.currency_id
    ...
    def lines(self, main_account):
        ...
        for l in res:
            l["move"] = (
                l["move_name"] != "/" and l["move_name"] or ("*" + str(l["mmove_id"]))
            )
    ...
    def _get_account(self, data):
        if data["model"] == "account.account":
            return (
                self.pool.get("account.account")
                .browse(self.cr, self.uid, data["form"]["id"])
                .company_id.name
            )
