...
class TestRegistry(AccountingTestCase):
    ...
    def test_invoice_and_report_cash2(self):
        ...
        journal_cash = self.env["account.journal"].search([("type", "=", "general")])[0]

        settings = self.env["account.config.settings"].search([("company_id", "=", 1)])
        ...
        company.write({"tax_cash_basis_journal_id": journal_cash.id})
        ...
        journal_cash = self.env["account.journal"].search([("type", "=", "cash")])[0]

        payment_methods = (
            journal.inbound_payment_method_ids or journal.outbound_payment_method_ids
        )
