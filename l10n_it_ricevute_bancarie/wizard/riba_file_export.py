...
##############################################################################
#
#    Copyright (C) 2011-2012 Associazione Odoo Italia
...
class riba_file_export(orm.TransientModel):
    ...
    def _Record14(
        ...
        return " 14" + str(self._progressivo).rjust(
            7, '0') + " " * 12 + scadenza + "30000" + str(int(round(
                importo * 100))).rjust(13, '0') + "-" + abi_assuntrice.rjust(
            5, '0') + cab_assuntrice.rjust(5, '0') + conto.ljust(
            12, '0') + abi_domiciliataria.rjust(
            5, '0') + cab_domiciliataria.rjust(5, '0') + " " * 12 + str(
            sia_credit).rjust(
            5, '0') + "4" + codice_cliente.ljust(
            16) + " " * 6 + self._valuta + "\r\n"
    ...
    def _Record20(
        ...
        return " 20" + str(self._progressivo).rjust(7, '0') + self._creditore[
            0:24] + indirizzo_creditore.ljust(24)[0:
                                                  24] + cap_citta_creditore.ljust(24)[0:
                                                                                      24] + ref_creditore.ljust(24)[0:
                                                                                                                    24] + " " * 14 + "\r\n"

    def _Record30(self, nome_debitore, codice_fiscale_debitore):
        return " 30" + str(self._progressivo).rjust(
            7, '0') + nome_debitore.ljust(60)[0:
                                              60] + codice_fiscale_debitore.ljust(16, ' ') + " " * 34 + "\r\n"
    ...
    def _Record40(
        ...
        return " 40" + str(self._progressivo).rjust(
            7, '0') + indirizzo_debitore.ljust(30)[0:30] + str(cap_debitore).rjust(
            5,
            '0') + self._comune_provincia_debitor + descrizione_domiciliataria.ljust(50)[0:50] + "\r\n"
    ...
    def _Record50(
        ...
        return " 50" + str(self._progressivo).rjust(
            7, '0') + self._descrizione.ljust(80)[0:
                                                  80] + " " * 10 + partita_iva_creditore.ljust(
            16, ' ') + " " * 4 + "\r\n"

    def _Record51(self, numero_ricevuta_creditore):
        return " 51" + str(self._progressivo).rjust(7, '0') + str(
            numero_ricevuta_creditore).rjust(10, '0') + self._creditore[0:
                                                                        20] + " " * 80 + "\r\n"
    ...
    def _creaFile(self, intestazione, ricevute_bancarie):
        accumulatore = self._RecordIB(intestazione[0], intestazione[
            1], intestazione[4], intestazione[5],
            intestazione[6])
        for value in ricevute_bancarie:  # estraggo le ricevute dall'array
            self._progressivo += 1
            if not value[9] or not value[10]:
                raise osv.except_osv('Error', _(
                    'No ABI / CAB specified for bank in ') + value[3])
            accumulatore += self._Record14(
                value[1], value[2], intestazione[1], intestazione[
                    2], intestazione[3], value[9], value[10],
                intestazione[0], value[12])
            accumulatore += self._Record20(intestazione[7], intestazione[
                8], intestazione[9],
                intestazione[10])
            accumulatore += self._Record30(value[3], value[4])
            accumulatore += self._Record40(value[5], value[6], value[7], value[
                8], value[11])
            accumulatore += self._Record50(value[2], value[13], value[
                14], intestazione[11], value[15],
                value[16])
    ...
    def act_getfile(self, cr, uid, ids, context=None):
        ...
        er_obj = self.pool['riba.distinta'].browse(
        ...
        if not credit_bank.codice_sia:
            raise osv.except_osv('Error', _(
                'No SIA Code specified for: ') + name_company)
        ...
        if not creditor_address[0].street:
            raise osv.except_osv('Error', _(
                'No address specified for: ') + name_company)
        creditor_city = ''
        ...
        if not order_obj.config.company_id.partner_id.vat and not order_obj.config.company_id.partner_id.fiscalcode:
            raise osv.except_osv('Error', _(
                'No VAT or Fiscalcode specified for: ') + name_company)
        array_testata = [
            credit_sia,
            credit_abi,
            credit_cab,
            ...
            creditor_address[0].zip and creditor_address[0].zip.replace(
                'x', '0')[0:5] or '' + ' ' + creditor_city, or
            er_obj.config.company_id.partner_id.ref or '', or
            er_obj.config.company_id.partner_id.vat and order_obj.config.company_id.partner_id.vat[2:] or order_obj.config.company_id.partner_id.fiscalcode,
        ]
        arrayRiba = []
        for line in order_obj.line_ids:
            ...
            if line.bank_riba_id:
                ...
                if debit_riba_bank.abi and debit_riba_bank.cab:
                    ...
                    if not debit_abi or not debit_cab:
                        raise osv.except_osv('Error', _(
                            'No bank or IBAN specified for ') + line.bank_riba_id.name)
            ...
            if debitor_address[0].city:
                ...
                raise osv.except_osv('Error', _(
                    'No City specified for ') + line.partner_id.name)
            debitor_province = ''
            ...
            if not line.due_date:  # ??? VERIFICARE
                ...
                due_date = datetime.strptime(line.due_date[:
                                                           10], '%Y-%m-%d').strftime("%d%m%y")

            if not line.partner_id.vat and not line.partner_id.fiscalcode:
                raise osv.except_osv('Error', _(
                    'No VAT or Fiscal code specified for ') + line.partner_id.name)
            if not debit_bank_name:
                # .bank and debit_bank.bank.name or debit_bank.bank_name):
                raise osv.except_osv('Error', _(
                    'No debit_bank specified for ') + line.partner_id.name)
            cup = ''
            cig = ''
            if line.cup:
        ...
        out = base64.encodestring(self._creaFile(
            array_testata, arrayRiba).encode("iso-8859-1"))

        return self.write(cr, uid, ids, {'state': 'get', 'riba_.txt':
                                         out}, context=context)
