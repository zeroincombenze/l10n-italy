...
class riba_accreditation(orm.TransientModel):
    ...
    def _get_accreditation_amount(self, cr, uid, context=None):
        ...
        if context.get('active_model', False) == 'riba.distinta.line':
            ...
            distinta_lines = distinta_line_pool.browse(cr, uid, context[
            ...
            for line in distinta_lines:
                ...
                if line.distinta_id.config != config:
                    raise osv.except_osv(_('Error'), _(
                        'Accredit only one bank configuration is possible'))
            ...
            distinta = distinta_pool.browse(cr, uid, context[
                'active_id'], context=context)
            for line in distinta.line_ids:
                if line.tobeaccredited and line.state in [
    ...
    _columns = {
        ...
        'acceptance_account_id': fields.many2one(
            'account.account', "Ri.Ba. acceptance account"),
    ...
    def create_move(self, cr, uid, ids, context=None):
        ...
        if context.get('active_model', False) == 'riba.distinta':
            ...
            if not context.get(
                'accruement',
                    False) and not distinta.config.accreditation_account_id:
                context.update({'accruement': True, 'accreditation_accruement':
                                True})
            ref = distinta.name
...
#        move_line_pool = self.pool.get('account.move.line')
        ...
        if context.get('active_model', False) == 'riba.distinta.line':
            ...
            if not context.get(
                'accruement',
                    False) and not line.distinta_id.config.accreditation_account_id:
                context.update({'accruement': True, 'accreditation_accruement':
                                True})
        ...
        if not context.get('accruement', False):
            if not wizard.bank_account_id or not wizard.accreditation_account_id:
                raise osv.except_osv(_('Error'), _(
                    'Bank account is mandatory for accreditation move'))
        if context.get('accruement', False):
            if not wizard.acceptance_account_id and not wizard.accreditation_account_id:
                raise osv.except_osv(_('Error'), _(
                    'Acceptance or accredit account is mandatory for accrue move'))
            ...
            'line_id': [
                (0, 0, {
                    ...
                    'account_id': context.get(
                        ...
                        'accreditation_accruement',
                        False) and wizard.accreditation_account_id.id or wizard.bank_account_id.id,
                ...
                (0, 0, {
                    ...
                    'account_id': context.get(
                        'accruement',
                        False) and wizard.acceptance_account_id.id or wizard.accreditation_account_id.id,
            ...
            if context.get('accruement', False):
                ...
                for line in distinta_lines:
                    if not line.state == "accredited":
                        line.write({'accreditation_move_id': move_id,
                                    'state': 'accredited'})
            # TODO: if all lines of a distinta are accredited, set
            # distinta accredited
        return {
