...
class AccountMoveLine(models.Model):
    ...
    iban = fields.Char(
        related="partner_id.bank_ids.acc_number", string="IBAN", store=False, copy=False
    )
...
class AccountInvoice(models.Model):
    ...
    unsolved_move_line_ids = fields.Many2many(
        ...
        "invoice_id",
        "line_id",
        "Unsolved journal items",
        copy=False,
    )
    is_unsolved = fields.Boolean(
        "The unsolved is open", compute="_compute_is_unsolved", store=True, copy=False
    )
