...
class WebsiteSaleCorrispettivi(WebsiteSale):
    def _checkout_form_save(self, mode, checkout, all_values):
        res = super(WebsiteSaleCorrispettivi, self)._checkout_form_save(
            mode, checkout, all_values
        )
        use_invoice = all_values.get("use_invoice")
        # Update order
        order = request.website.sale_get_order()
        order_values = self._prepare_corrispettivi_order_values(order, use_invoice)
    ...
    def _prepare_corrispettivi_partner_values(self, partner, use_invoice):
        ...
        if not partner.property_account_position_id:
            ...
            if not use_invoice:
                company = (
                    partner.company_id
                    or partner.default_get(["company_id"])["company_id"]
                )
    ...
    def _prepare_corrispettivi_order_values(self, order, use_invoice):
        ...
        if not order.fiscal_position_id:
            ...
            if not use_invoice:
                company = (
                    order.company_id or order.default_get(["company_id"])["company_id"]
                )
